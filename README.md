# Yolo Basin Storymap

**Live link to beta version (under active development): https://skylerlewis.github.io/yolo-basin/**

[<img src="./media/ybf-logo.jpg" title="Yolo Basin Foundation Logo" height="100px" />](https://yolobasin.org)

## Credits and licenses

Content including text, images, and map layers is &copy; Yolo Basin Foundation unless otherwise credited in image bylines and map footnotes.

Software is modified from a [template](https://github.com/HandsOnDataViz/leaflet-storymaps-with-google-sheets) from [HandsOnDataViz](https://github.com/HandsOnDataViz) developed by [Ilya Ilyankou](https://github.com/ilyankou) and [Jack Dougherty](https://github.com/jackdougherty) with support from Connecticut Humanities and Trinity College, CT. Inspired by Code for Atlanta mapsfor.us (2016) https://github.com/codeforatlanta/mapsforus (BSD-3-Clause). Adapted from MUX Lab, Map Effects 100: https://github.com/muxlab/map-effects-100, see http://muxlab.github.io/map-effects-100/Leaflet/11_scroll-driven-map-navigation.html. Components include:
- Leaflet v1.7.1 https://leafletjs.com (BSD-2-Clause)
- jQuery v3.5.1 https://code.jquery.com (MIT)
- PapaParse v5.3.0 to parse CSV with JavaScript (MIT)
- Font Awesome v5.8.1 https://cdn.fontawesome.com (MIT, SIL OFL 1.1)
- leaflet-providers (v1.10.2) https://github.com/leaflet-extras/leaflet-providers (BSD-2-Clause)
- Leaflet.awesome-markers (v2.0.4), manually updated to svg to allow hex and material icons https://github.com/sigma-geosistemas/Leaflet.awesome-markers (MIT)
- Leaflet.ExtraMarkers (v1.0.5) https://github.com/coryasilva/Leaflet.ExtraMarkers (MIT)
- jQuery-CSV (v1.0.11) https://github.com/evanplaice/jquery-csv (MIT)
- Single Element CSS Spinner (31 May 2016) https://github.com/lukehaas/css-loaders (MIT)
- Lightbox by Lokesh Dhakar (v.2.11.3) https://github.com/lokesh/lightbox2 (MIT)
- [Google Sheets API version 4](https://developers.google.com/sheets/api)
- via [Google Fonts](https://developers.google.com/fonts): [Libre Franklin](https://github.com/impallari/Libre-Franklin) by [Impallari Type](https://github.com/impallari) 
 ([OFL](https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL))
